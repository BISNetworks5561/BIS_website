import Button from "@/components/ui/Button";
import Icon from "@/components/ui/Icon";
import UplusBadge from "@/components/ui/UplusBadge";
import { cctvHero as hero } from "@/lib/content-cctv";
import { siteConfig, telHref } from "@/site.config";

export default function CctvHero() {
  const best = siteConfig.pricingCctv.terms.find((t) => t.popular) ?? siteConfig.pricingCctv.terms[0];
  return (
    <section className="relative overflow-hidden border-b border-line bg-white md:mt-6 md:rounded-3xl md:border">
      <div aria-hidden className="pointer-events-none absolute -right-32 -top-32 h-[480px] w-[480px] rounded-full bg-brand-light blur-3xl md:h-[640px] md:w-[640px]" />
      <div className="relative mx-auto grid max-w-6xl gap-10 px-5 py-16 md:px-8 md:py-20 lg:grid-cols-[1.1fr_0.9fr] lg:items-center lg:py-24">
        <div className="animate-fade-up">
          <p className="mb-4">
            <UplusBadge>{hero.eyebrow}</UplusBadge>
          </p>
          <h1 className="text-3xl font-black leading-[1.2] tracking-tight md:text-4xl md:leading-[1.15] lg:text-5xl">
            {hero.title[0]}
            <br />
            <span className="text-brand-gradient">{hero.title[1]}</span>
          </h1>
          <p className="mt-5 max-w-xl text-base text-muted md:text-lg">{hero.subtitle}</p>
          <ul className="mt-6 flex flex-wrap gap-x-5 gap-y-2">
            {hero.bullets.map((b) => (
              <li key={b} className="flex items-center gap-1.5 text-sm font-semibold">
                <Icon name="check" className="h-4 w-4 text-brand" strokeWidth={2.5} />
                {b}
              </li>
            ))}
          </ul>
          <div className="mt-8 flex flex-col gap-3 sm:flex-row">
            <Button href="#consult" size="lg">
              {hero.primaryCta}
              <Icon name="arrow" className="h-5 w-5" />
            </Button>
            <a
              href={telHref(siteConfig.contact.phone)}
              className="inline-flex h-14 items-center justify-center gap-2 rounded-full border-2 border-brand-dark px-8 text-base font-bold text-brand-dark hover:bg-brand-dark hover:text-white"
            >
              <Icon name="phone" className="h-5 w-5" />
              {hero.secondaryCta} {siteConfig.contact.phoneDisplay}
            </a>
          </div>
          <p className="mt-4 text-sm text-muted">{hero.note}</p>
        </div>

        <div className="animate-fade-up [animation-delay:120ms]">
          <div className="rounded-3xl border border-line bg-white p-6 shadow-card md:p-8">
            <UplusBadge>U+지능형CCTV</UplusBadge>
            <p className="mt-1 text-2xl font-black">{hero.card.title}</p>
            <dl className="mt-6 grid grid-cols-2 gap-4">
              {hero.card.stats.map((d) => (
                <div key={d.k} className="rounded-2xl bg-surface p-4">
                  <dt className="text-xs font-medium text-muted">{d.k}</dt>
                  <dd className="mt-1 text-xl font-black">{d.v}</dd>
                </div>
              ))}
            </dl>
            <div className="mt-6 flex items-center justify-between rounded-2xl bg-brand-gradient p-4 text-white">
              <div>
                <p className="text-xs opacity-80">카메라 1대당 · {best.term}</p>
                <p className="text-2xl font-black">
                  월 {best.price.toLocaleString("ko-KR")}원<span className="text-sm font-medium opacity-80">부터</span>
                </p>
              </div>
              <span className="rounded-full bg-white/15 px-3 py-1 text-xs font-bold ring-1 ring-white/30">부가세 포함</span>
            </div>
          </div>
        </div>
      </div>
    </section>
  );
}
